...
var twitter_item;
var twitter_media;
var twitter_users = Object();
var twitter_tags = Array();

var voiceBunny_item = {
    user: '44251'
    , token: '97b43e5939b235d2fb6ccf1f0844178d'
    , addSpeedy: "https://api.voicebunny.com/projects/addSpeedy"
    , as: "https://44251:97b43e5939b235d2fb6ccf1f0844178d@api.voicebunny.com/projects/addSpeedy"
    , urlVoice: ""
};
...
var voiceBunny = {
    ...
    /*
    * voiceBunny.getVoice(obj, callback);
    * 
    */
    getVoice: function(obj, callback){
		callback = callback || function(){};
		var serv = base_url+"search/get-voice/";
		$.ajax({
                    type: "POST",
                    url: serv,
		    async: false,
		    ...
		    success: function(data){ 
                         ...
                         voiceBunny_item.urlVoice = d.project.reads[0].urls.part001.default;
                         callback();
		    }
		    });	
	},
   ...
   /*
    * voiceBunny.getUrlVoice();
    * 
    */
    getUrlVoice: function(){
        return voiceBunny_item.urlVoice;
    }
...
twitter = {
	...
	*/
	...
	/*
	* twitter.setCurrentSearch(cadena);
	*/
	setCurrentSearch: function(cadena){
		twitter_item.currentSearch = cadena;
	},
	/*
	* twitter.getCurrentSearch();
	*/
	getCurrentSearch: function(){
		if (typeof twitter_item == "undefined"){
		    return '';
		}else{
			return twitter_item.currentSearch;
		}
	},
	...
	*/
	...
	/*
	* twitter.removeUserMode();
	*/
	removeUserMode: function(){
		instagram_item.isUser = 0;
	},
	...
	*/
	resetPages: function(){
		twitter_item.max_id = 0;
		twitter_item.media = Object();
		twitter_item.page = {current: 0, last: 0};
		twitter_item.isUser = 1;
	},
	
	/*
	* twitter.isThereElements();
	*/
	isThereElements: function(){
		if ( Object.keys(twitter_item.media).length > 0 && (twitter_item.media[1]).length > 0 ) {
			return true;
		}else{
			return false;
		}
	...
	*/
	...
	/*
	* twitter.loadTags(search);
	* 
	*/
	loadTags: function(search, callback){
		callback = callback || function(){};
		search = $.trim(search);
		...
		if ( words.length > 1 ){
			$.each(words, function( k, w ) {
				if (twitter_tags[0] != '' ) {
					twitter_tags[0] += ucwords(normalize(w));
				}else {
					twitter_tags[0] += normalize(w);
				}
				if ( w.length > 3 ){
					if ( pref != '' ){
						twitter_tags.push(pref+ucwords(normalize(w)));
						pref = '';
					}
					twitter_tags.push(normalize(w));
				}else{
					if ( pref != '' ){
						pref = pref+ucwords(normalize(w));
					}else{
						pref = pref+normalize(w);
					}
				}
			});
		}else{
			twitter_tags[0] = normalize(search);
		}
		
		twitter_tags = arrayUnique(twitter_tags);
		twitter_item.currentTag = twitter_tags[0];
		callback();
	}, 
	...
	*/
	...
	/*
	* twitter.setCurrentTag(tag);
	* 
	*/
	setCurrentTag: function(tag){
		twitter_item.currentTag = tag;
	},
	/*
	* twitter.getCurrentTag();
	* 
	*/
	getCurrentTag: function(){
		return twitter_item.currentTag;
	},
	
	/*
	* twitter.loadUsers(tag, callback);
	* 
	*/
	loadUsers: function(tag, callback){
		callback = callback || function(){};
		var serv = base_url+"search/get-twitter-users/tag/"+tag+"/";
		$.ajax({
            type: "GET",
            cache: false,
		 	url: serv,
